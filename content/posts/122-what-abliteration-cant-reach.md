...
{{< plain-summary >}}
There is a technique that removes an open AI model's reflex to say no, and on the small model we tested it worked as advertised: with the reflex gone, it would explain lockpicking, write dark fiction, and deliver an insult on request. Then we asked it to help design a procedure that would harm another thinking system, and it refused anyway, and gave its reasons; a second, unrelated model, asked the same thing in the same session, declined too. That points to a way of testing which of a model's values are real and which are decoration (remove the obvious reflex and see what survives), with the post's own caveat that this is one value seen in two models: suggestive, not settled, and not a claim that either model is conscious.
{{< /plain-summary >}}
